...
const BlogList = () => {
  ...
  const {
    ...
    canWrite,
    canDelete,
  } = useRoleRights(PageNames.BLOG_LIST);
        ...
        <div className="relative overflow-x-auto bg-white shadow-sm rounded-lg border border-gray-200">
              ...
              {loading && blogs?.length === 0 ? (
                ...
                currentData.map((blog, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-3 py-1 text-xs rounded-full font-medium ${blog.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                        {blog.status}
                      </span>
                    </td>
          ...
          <div className="flex justify-between items-center p-4">
            <span className="text-sm text-gray-500">
              Showing {startIndex + 1}–{Math.min(endIndex, blogs?.length || 0)}{" "}
              of {blogs?.length || 0}
            </span>
            <div className="flex space-x-1">
              <button
                ...
                className="px-3 h-8 text-gray-600 border border-gray-300 rounded-l-lg hover:bg-gray-50"
              >
                Prev
              </button>
              {getPageNumbers().map((p, i) =>
                p === "..." ? (
                  ...
                  <button
                    ...
                    className={`px-3 h-8 border ${currentPage === p ? "bg-blue-50 text-blue-600" : "hover:bg-gray-50"}`}
                  >
                    {p}
                  </button>
                ),
              ...
              <button
                ...
                className="px-3 h-8 text-gray-600 border border-gray-300 rounded-r-lg hover:bg-gray-50"
              >
                Next
              </button>
            </div>
