...
const AddOccupation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await dispatch(getAllOccupations()).unwrap();
      resetForm();
      setCurrentPage(1);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = (id) => {
    const currentUserId = authUser?.id || null;
    dispatch(deleteOccupation({ id, user_id: currentUserId })).then(() => {
  ...
  /* ===== PAGINATION LOGIC ===== */
  const totalPages = Math.ceil((occupations?.length || 0) / itemsPerPage);
...
hover:after:w-full"
          ...
          <div className="flex justify-between items-center p-4">
            <span className="text-sm text-gray-500">
              ...
              {Math.min(endIndex, occupations?.length || 0)} of{" "}
