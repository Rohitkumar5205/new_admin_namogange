...
} from "../../redux/slices/add_by_admin/coordinatorStatusSlice";
...
const CoordinatorStatus = () => {
  ...
  /* ===== FETCH DATA ===== */
  useEffect(() => {
    dispatch(getAllCoordinatorStatus());
  }, [dispatch]);

  const { canRead, canWrite, canDelete, isFormDisabled } = useRoleRights(PageNames.ADD_COORDINATOR_STATUS);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        ...
        showSuccess("Coordinator Status updated successfully");
      } else {
        await dispatch(
          createCoordinatorStatus({
            title: formData.title,
        ...
        showSuccess("Coordinator Status added successfully");
      }
      await dispatch(getAllCoordinatorStatus()).unwrap();
      resetForm();
      setCurrentPage(1);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = (id) => {
    ...
    dispatch(deleteCoordinatorStatus({ id, user_id: currentUserId })).then(
      () => {
        showSuccess("Coordinator Status deleted successfully");
        dispatch(getAllCoordinatorStatus());
      }
    );
  };
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col text-center">
              ...
              <p className="text-sm text-blue-100">
                Add or update Coordinator Status content including title,
                status.
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Coordinator Status Title <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                name="title"
          ...
          <table className="w-full text-sm text-left text-gray-600">
            <thead className="bg-gray-50 border-b  border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Coordinator Status Title</th>
...
export default CoordinatorStatus;
