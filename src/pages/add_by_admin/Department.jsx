...
import {
  ...
  updateDepartment,
  deleteDepartment,
} from "../../redux/slices/add_by_admin/departmentSlice";
...
const Department = () => {
  ...
  const { canRead, canWrite, canDelete, isFormDisabled } = useRoleRights(PageNames.ADD_DEPARTMENT);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      showError("Department title is required.");
      return;
    }
  ...
  const handleDelete = (id) => {
    const currentUserId = authUser?.id || null;
    dispatch(deleteDepartment({ id, user_id: currentUserId })).then(() => {
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col text-center">
              ...
              <p className="text-sm text-blue-100">
                Add or update Department content including title, image, link
                and status.{" "}
              </p>
            </div>
          </div>
          ...
          <table className="w-full text-sm text-left text-gray-600">
            <thead className="bg-gray-50 border-b  border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Department Title</th>
...
hover:after:w-full"
          ...
          <div className="flex justify-between items-center p-4">
            <span className="text-sm text-gray-500">
              ...
              {Math.min(endIndex, departments?.length || 0)} of{" "}
